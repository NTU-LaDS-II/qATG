...
class qatg():
	...
	def getTestConfiguration(self, faultList, simulateConfiguration: bool = True):
		...
		for k in range(len(faultList)):
			...
			template = self.generateTestTemplate(faultObject = fault, initialState = initialState)
			configurationList[k].setTemplate(template)
	...
	def findNewElement(self, faultObject, faultyQuantumState, faultfreeQuantumState):
		...
		# iterate by qubit
		newElement = [list(x) for x in zip(*newElement)]
		# iterate by order
