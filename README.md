...
This is an quantum automatic test generator, compatible with __**Qiskit version 1.0**__ to __**2.0**__. The generator generates a configuration for each user-defined fault.
...
> **📢 Note for OTEM features:** > We have implemented additional features specifically for Online Testing Error Mitigation (OTEM). If you are looking for OTEM-related functionalities, please check out the [OTEM branch](https://github.com/NTU-LaDS-II/qATG/tree/OTEM).
...
cd qatg
```
(If you want to use the additional Online Testing Error Mitigation features, please switch to the OTEM branch by running git checkout OTEM before installation).
