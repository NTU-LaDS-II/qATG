...
class ATPG():
    ...
    def get_quantum_gate(self, gate_type, index, parameter=[]):
        ...
        if(gate_type == Qgate.U3Gate):
        	return QuantumGate(gate_type(parameter[0], parameter[1], parameter[2]), [qiskit.circuit.quantumregister.Qubit(QuantumRegister(self.circuit_size, self.qr_name), index[0])], [])
        elif(gate_type == Qgate.CXGate):
        	return QuantumGate(gate_type() , [qiskit.circuit.quantumregister.Qubit(QuantumRegister(self.circuit_size, self.qr_name), index[0]), 
                            qiskit.circuit.quantumregister.Qubit(QuantumRegister(self.circuit_size, self.qr_name), index[1])], [])
        elif(gate_type == Qgate.Barrier):
        	return QuantumGate(gate_type(1), [qiskit.circuit.quantumregister.Qubit(QuantumRegister(self.circuit_size, self.qr_name), index[0])], [])
    ...
    def build_two_configuration(self, template, fault_list):
            ...
            for fault_index in index_array: ##這組電路幾個pair
                ...
                if fault_index == index_pair:
                    while gate_index < len(template[0]):
                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[0][gate_index][1]), index=fault_index[0], parameter=template[0][gate_index][1].params))                
                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[0][gate_index][0]), index=fault_index[1], parameter=template[0][gate_index][0].params))

                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[0][gate_index+1]), index=fault_index[0], parameter=template[0][gate_index+1].params))
                
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[0], parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[1], parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[0][gate_index+2]), index=fault_index, parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[0], parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[1], parameter=[]))

                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[0][gate_index+3]), index=fault_index[1], parameter=template[0][gate_index+3].params))
                        gate_index += 4
                else:
                    while gate_index < len(template[1]):
                        # print(fault_index)
                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[1][gate_index][1]), index=fault_index[0], parameter=template[1][gate_index][1].params))                
                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[1][gate_index][0]), index=fault_index[1], parameter=template[1][gate_index][0].params))
                
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[0], parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[1], parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(template[1][gate_index+1]), index=fault_index, parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[0], parameter=[]))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault_index[1], parameter=[]))
                        gate_index += 2
        ...
        while gate_index < len(template[1]):
            for fault in fault_list:##這組電路幾個pair
                qc_faultfree._data.append(self.getQuantumGate(gate_type=type(template[1][gate_index][1]), index=fault.index[0], parameter=template[1][gate_index][1].params))                
                qc_faultfree._data.append(self.getQuantumGate(gate_type=type(template[1][gate_index][0]), index=fault.index[1], parameter=template[1][gate_index][0].params))
                
                qc_faultfree._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault.index[0], parameter=[]))
                qc_faultfree._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault.index[1], parameter=[]))
                qc_faultfree._data.append(self.getQuantumGate(gate_type=type(template[1][gate_index+1]), index=fault.index, parameter=[]))
                qc_faultfree._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault.index[0], parameter=[]))
                qc_faultfree._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=fault.index[1], parameter=[]))
            gate_index += 2
    ...
    def build_single_configuration(self, template, fault_list):
        ...
        for num_circuit in range(self.circuit_size):
            ...
            for gate in template[0]:
                qc_faulty._data.append(self.getQuantumGate(gate_type=type(gate), index=num_circuit, parameter=gate.params))
                qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=num_circuit, parameter=[]))

            for gate in template[1]:
                for n in range(self.circuit_size):
                    if n != num_circuit:
                        qc_faulty._data.append(self.getQuantumGate(gate_type=type(gate), index=n, parameter=gate.params))
                        qc_faulty._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=n, parameter=[]))
        ...
        for gate in template[1]:
            for n in range(self.circuit_size):
                qc_faultfree._data.append(self.getQuantumGate(gate_type=type(gate), index=n, parameter=gate.params))
                qc_faultfree._data.append(self.getQuantumGate(gate_type=Qgate.Barrier, index=n, parameter=[]))
        qc_faultfree.measure(self.quantumregister, self.classicalregister)
        # print("faultfree")
        # print(qc_faultfree)
        new_configuration = Configuration(qc_faulty_list, qc_faultfree, length, fault_list)
