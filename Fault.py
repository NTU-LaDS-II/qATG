...
class Fault():
	...
	#	if type(gate_info) !=  self.gate_type:
	#		return False
	#	else:
...
class U_variation_fault(Fault):
	def __init__(self, index, ratio=[1, 1, 1], bias=[0, 0, 0]):
		...
		#彈性空間，由ratio的len決定
	def get_faulty_gate(self , gate_info):
		#faulty_gate有不必要資訊，例如index和空list
		faulty_gate = deepcopy(gate_info)
		for i in range(len(self.ratio)):
			faulty_gate.params[i] = self.ratio[i]*faulty_gate.params[i] + self.bias[i]
		return faulty_gate
...
class U_threshold_lopa(Fault):
	...
	def get_faulty_gate(self, gate_info):
		...
		for i in range(len(self.threshold)):
			faulty_gate.params[i] = self.threshold[i] if faulty_gate.params[i] > self.threshold[i] else faulty_gate[0].params[i]
		return faulty_gate
