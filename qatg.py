...
class qatg():
	...
	def __init__(self, \
			...
			gridSlice: int = 21, gradientDescentMaxIteration: int = 800, \
			gradientDescentStep: float = 0.5, gradientMeasureStep: float = 0.001, gradientDeltaThreshold: float = 1e-8, \
	...
	def findNewElement(self, faultObject, faultyQuantumState, faultfreeQuantumState):
		...
		# print(f"Before: {optimalParameterSet}")
		...
		for k in range(self.gradientDescentMaxIteration):
			...
			for m in range(qubitSize):
				for n in range(3):
					...
					deltaParameter = (score(tempParameterSet) - currentOptimalScore) / self.gradientMeasureStep
					tempParameterSet[m][n] -= self.gradientMeasureStep
					optimalParameterSet[m][n] += self.gradientDescentStep * deltaParameter
					maxDelta = abs(deltaParameter) if abs(deltaParameter) > maxDelta else maxDelta
			if maxDelta < self.gradientDeltaThreshold:
				break
		print(k)
		# print(f"After : {optimalParameterSet}")
